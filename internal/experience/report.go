...
func WriteReport(outputDir string, report Report) error {
	if !filepath.IsAbs(outputDir) { return errors.New("report output must be an absolute caller-owned directory") }
	if err := os.MkdirAll(outputDir, 0o755); err != nil { return err }
	data, err := report.JSON(); if err != nil { return err }
...
func RenderMarkdown(report Report) string {
	...
	for _, item := range report.CanonicalCases { fmt.Fprintf(&b, "| %s | %s | %s | %s |\n", item.CaseID, item.Class, item.State, item.Reason) }
	b.WriteString("\n## UNKNOWN coordinates\n\n")
	for _, item := range report.CanonicalCases { if item.Unknown != nil { u := item.Unknown; fmt.Fprintf(&b, "- `%s`: stage=`%s`; step=`%s`; reason=`%s`; unknown_class=`%s`; next_operation=`%s`; blocked_by=`%s`\n", item.CaseID, u.Stage, u.Step, u.Reason, u.UnknownClass, u.NextOperation, strings.Join(u.BlockedBy, ",")) } }
	b.WriteString("\n## 12-cell bindings\n\n")
	for _, binding := range report.Bindings { fmt.Fprintf(&b, "- `%02d %s` -> IR `%s` -> generated `%s` -> evaluator `%s`\n", binding.Ordinal, binding.Activity, binding.IRNode, binding.GeneratedSymbol, binding.Evaluator) }
