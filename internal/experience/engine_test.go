...
func TestAssessCandidateRequiresTheCompleteSemanticMatch(t *testing.T) {
	...
	if decision.State != StateRefuted || decision.Decision != "AVOID" || !decision.MatchBasis.AllThreeExact { t.Fatalf("exact match = %#v", decision) }
	drift := candidate
	...
	if driftDecision.State != StateUnknown || driftDecision.Unknown == nil || driftDecision.Unknown.UnknownClass != UnknownMismatch { t.Fatalf("fingerprint drift = %#v", driftDecision) }
}
...
func TestStaleReceiptIsUnknownAndContradictionIsRefuted(t *testing.T) {
	...
	if stale.State != StateUnknown || stale.Unknown == nil || stale.Unknown.UnknownClass != UnknownStale { t.Fatalf("stale = %#v", stale) }
	record := MemoryRecord{Schema: MemorySchema, RecordID: "record-1", Ordinal: 1, Kind: StateRefuted, FixtureDigest: fixtureDigest, SemanticFingerprint: fingerprint, FailureClass: "KNOWN", ScopeDigest: scope, OutcomeReceipt: receipt}
	record.RecordDigest = DigestRecord(record)
	...
	if contradiction.State != StateRefuted || contradiction.Reason != "CURRENT_OBSERVATION_CONTRADICTS_KNOWN_REFUTATION" { t.Fatalf("contradiction = %#v", contradiction) }
}
