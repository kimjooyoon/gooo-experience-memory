...
func LoadMeta(sourcePath, contractPath, irPath, generatedPath string) (Meta, error) {
	...
	var ir SemanticIR
	if err := LoadJSON(irPath, &ir); err != nil {
		return Meta{}, err
	}
...
func Evaluate(meta Meta, fixture FixedFixture, records []MemoryRecord, receipt *OutcomeReceipt, cases []CanonicalCase, runtime RuntimeInput) (Report, error) {
	started := time.Now()
	if err := ValidateDenominator(meta.Contract); err != nil {
		return Report{}, err
	}
	...
	if len(cases) != 12 {
		return Report{}, errors.New("canonical case denominator must be exactly 12")
	}
	for _, record := range records {
		if err := ValidateRecord(record, FixtureDigest(fixture)); err != nil {
			return Report{}, err
		}
	}
	if receipt != nil && receipt.FixtureDigest != FixtureDigest(fixture) {
		return Report{}, errors.New("receipt is bound to another fixture")
	}
	...
	for _, item := range cases {
		...
		if result.State == StateUnknown && !validUnknown(result.Unknown) {
			return Report{}, fmt.Errorf("canonical case %s has incomplete UNKNOWN tuple", item.CaseID)
		}
		canonical = append(canonical, result)
		counts[item.Class]++
	}
	if counts["normal"] != 4 || counts[StateUnknown] != 4 || counts[StateRefuted] != 4 {
		return Report{}, errors.New("canonical denominator must be normal=4 UNKNOWN=4 REFUTED=4")
	}
	...
	if runtime.Authority != (Authority{}) && runtime.Authority != metrics.Authority {
		return Report{}, errors.New("authority observation is not zero")
	}
	return Report{
...
func fixtureDigestIsReferenced(fixture FixedFixture, records []MemoryRecord, receipt *OutcomeReceipt) bool {
	...
	if len(records) == 0 && receipt == nil {
		return true
	}
	for _, record := range records {
		if record.FixtureDigest != digest {
			return false
		}
	}
	if receipt != nil && receipt.FixtureDigest != digest {
		return false
	}
	return true
}
...
func AssessCandidate(candidate Candidate, records []MemoryRecord, receipt *OutcomeReceipt, fixtureDigest, fixtureVersion string, observed *Attempt) CandidateDecision {
	...
	related := false
	for _, record := range records {
		if record.FixtureDigest != fixtureDigest {
			continue
		}
	...
	if len(exact) == 1 {
		record := exact[0]
		if err := ValidateRecord(record, fixtureDigest); err != nil {
			return refutedDecision(decision, "TAMPERED_MEMORY_RECORD")
		}
	...
	if related {
		...
		if candidate.SemanticFingerprint != "" {
			reason = "SEMANTIC_FINGERPRINT_OR_SCOPE_DIGEST_DIFFERS_FROM_MEMORY"
		}
...
func refutedDecision(decision CandidateDecision, reason string) CandidateDecision {
	decision.State, decision.Decision, decision.Reason = StateRefuted, "AVOID", reason
	return decision
}

func unknownDecision(decision CandidateDecision, unknown UnknownTuple) CandidateDecision {
	decision.State, decision.Decision, decision.Reason, decision.Unknown = StateUnknown, "DEFER_UNKNOWN", unknown.Reason, &unknown
	return decision
}

func sameAttemptBasis(candidate Candidate, attempt Attempt) bool {
	return candidate.SemanticFingerprint == attempt.SemanticFingerprint && candidate.FailureClass == attempt.FailureClass && candidate.ScopeDigest == attempt.ScopeDigest
}
...
func selectBaseline(fixture FixedFixture) SelectionSnapshot {
	...
	for _, candidate := range candidates {
		decisions = append(decisions, CandidateDecision{CandidateID: candidate.CandidateID, State: StateClosed, Decision: "ELIGIBLE", Reason: "NO_MEMORY_INPUT", MatchBasis: MatchBasis{SemanticFingerprint: candidate.SemanticFingerprint, FailureClass: candidate.FailureClass, ScopeDigest: candidate.ScopeDigest}})
	}
...
func selectAfter(fixture FixedFixture, records []MemoryRecord, receipt *OutcomeReceipt) SelectionSnapshot {
	...
	for _, candidate := range candidates {
		...
		if selected == "" && decision.State == StateClosed {
			selected, selectedReason = candidate.CandidateID, "LOWEST_RANK_CANDIDATE_AFTER_EXPERIENCE_FILTER"
		}
	}
	state := StateUnknown
	if selected != "" {
...
func recurrenceCount(snapshot SelectionSnapshot, fixture FixedFixture, phase string) int {
	...
	for _, candidate := range fixture.Candidates {
		if candidate.CandidateID != snapshot.SelectedCandidateID {
			continue
		}
		for _, attempt := range fixture.Attempts {
			if attempt.Phase == phase && attempt.Outcome == StateRefuted && sameAttemptBasis(candidate, attempt) {
				return 1
			}
		}
	}
...
func unknownCount(snapshot SelectionSnapshot) int {
	count := 0
...
func classifyCase(item CanonicalCase, fixture FixedFixture, records []MemoryRecord, receipt *OutcomeReceipt, nodes []IRNode) CaseResult {
	...
	for index := range activities {
		if index < len(nodes) {
			activities[index] = nodes[index].Activity
		}
	}
	...
	case "FINGERPRINT_MISMATCH", "SCOPE_MISMATCH":
		candidate, ok := candidateByID(fixture.Candidates, item.CandidateID)
		if !ok {
			return refutedCase(result, "CASE_CANDIDATE_NOT_FOUND")
		}
		decision := AssessCandidate(candidate, records, receipt, FixtureDigest(fixture), fixture.Version, nil)
	...
	case "AMBIGUOUS_RECEIPT":
		candidate, ok := candidateByID(fixture.Candidates, "candidate-known-refuted")
		if !ok {
			return refutedCase(result, "CASE_CANDIDATE_NOT_FOUND")
		}
		duplicates := append([]MemoryRecord(nil), records...)
		if len(records) > 0 {
			duplicate := records[0]
			duplicate.RecordID = duplicate.RecordID + "-ambiguous"
...
func decisionCase(result CaseResult, decision CandidateDecision) CaseResult {
	result.State, result.Decision, result.Reason, result.Unknown = decision.State, decision.Decision, decision.Reason, decision.Unknown
	return result
}
func refutedCase(result CaseResult, reason string) CaseResult {
	result.State, result.Decision, result.Reason = StateRefuted, "REFUTED", reason
	return result
}
...
func validUnknown(unknown *UnknownTuple) bool {
	return unknown != nil && unknown.Stage != "" && unknown.Step != "" && unknown.Reason != "" && unknown.UnknownClass != "" && unknown.NextOperation != "" && unknown.BlockedBy != nil
}

func buildBindings(meta Meta) []Binding {
	bindings := make([]Binding, 0, len(meta.IR.Nodes))
	for _, node := range meta.IR.Nodes {
		bindings = append(bindings, Binding{Ordinal: node.Ordinal, CellID: meta.Contract.Cells[node.Ordinal-1].ID, Activity: node.Activity, SourcePath: meta.SourcePath, SourceLine: node.SourceLine, SourceDigest: meta.SourceDigest, IRNode: node.ID, GeneratedSymbol: node.Activity, GeneratedDigest: meta.GeneratedDigest, Evaluator: node.Evaluator, MetricID: node.MetricID})
	}
	return bindings
}

func balances(contract Denominator, proof bool) []Balance {
	values := contract.ProofChoices
	totals := contract.ProofTotals
	if !proof {
		values, totals = contract.IndicatorClasses, contract.IndicatorTotals
	}
	result := make([]Balance, 0, len(values))
	for _, value := range values {
		result = append(result, Balance{ChoiceOrClass: value, Total: totals[value]})
	}
	return result
}
...
func receiptDigest(receipt *OutcomeReceipt) string {
	if receipt == nil {
		return ""
	}
